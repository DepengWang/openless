...
- A foreground runtime supervisor (`OpenLessRuntimeService`) that keeps the process alive and holds the JNI Context every Rust→Kotlin call routes through, decoupled from any Activity's lifecycle. One tracked Tauri host Activity (`OpenLessBackendWarmupActivity`, also the launcher and settings-page host) is reused when possible and moved to the background during silent warmup; it now finishes outright when the user closes settings, and rebuilds its WebView on demand the next time it's needed.
...
- The Android input method covers voice, stroke, clipboard, and English input, with floating-capsule controls, tiered cross-app insertion (accessibility → Shizuku → clipboard), provider selection, local history, personalization, and runtime diagnostics. See the "Android input method" section above for the user-facing feature list.
- JNI Context registration (every Rust→Kotlin call) now lives on `OpenLessRuntimeService` (a `START_STICKY` foreground service tied to the IME's lifecycle), not on the settings/warmup Activity — a service is far more stable than an Activity that spends nearly its whole life backgrounded.
- `OpenLessBackendWarmupActivity.onCreate()` now calls a new Rust command, `ensure_main_webview_window()`, on every fresh instance — idempotent, it rebuilds the Tauri "main" `WebviewWindow` on demand when Wry's own Android `CreateWebView` dispatch wouldn't otherwise fire for a brand-new Activity instance (its `activity_id` never matches a prior `WEBVIEW_ATTRIBUTES` entry). Closing settings now calls `finishAndRemoveTask()` instead of `moveTaskToBack()`, verified on-device not to reproduce the historical HWUI/native-mutex crash risk that motivated `moveTaskToBack()` in the first place (that risk was specifically about backgrounding *during* cold-start init, not about finishing a fully-loaded session).
- A local, git-tracked vendor patch of `wry` 0.55.1 (`openless-all/app/src-tauri/vendor/wry-0.55.1/`, wired in via `[patch.crates-io]` the same way `vendor/wayland-scanner` already is) replaces two silent failure points in wry's Android `CreateWebView` dispatch — a `debug_assertions`-only `eprintln!()` that never reached `logcat`, and a `.expect()` that could panic across the JNI boundary — with proper `log::` calls, which is what made this session's diagnosis possible at all.

#### Known unresolved risks

- **Root-caused and fixed:** the settings-reopen black screen's primary cause was a Wry `activity_id` collision — `WryActivity.kt`'s `intent.extras?.getInt(ACTIVITY_ID_KEY) ?: hashCode()` never actually falls through to `hashCode()` once the Intent carries any extra (`Bundle.getInt()` returns 0, not null, for a missing key), so every Activity instance rebuilt via this app's own "start fresh" path collided on `activity_id=0`.
- **Root-caused and fixed:** a stale "main" window label left in Tauri's own `AppManager` registry after a dead Activity's WebView creation silently failed — `destroy()`/`close()` on the Rust `WebviewWindow` handle only clear `tauri-runtime-wry`'s own internal reference and are themselves asynchronous (they post a message to the tao event loop and return immediately); the label is only actually dropped in response to a genuine platform `WindowEvent::Destroyed`, which never fires for a webview that was never truly created. `ensure_main_webview_window()` now polls with a short bounded wait after `destroy()` before rebuilding, instead of racing it.
- **Not yet root-caused:** even with a unique `activity_id` and no stale label, Wry/Tauri's Android window-creation dispatch has been observed to silently never complete — `.build()` returns `Ok`, but the async chain that should follow (`InnerWebView::new` → `CreateWebView` → `onWebViewCreate()`) sometimes never runs, with no error surfaced anywhere. This appears to be an asynchronous dispatch issue inside `tauri-runtime-wry` itself (shared across all platforms, not Android-specific plumbing), and needs further investigation there specifically if it needs to be fully eliminated rather than just recovered from.
- **Safety nets, not fixes, for the residual issue above:** `OpenLessBackendWarmupActivity` now has a 4-second watchdog that finishes a settings Activity whose `onWebViewCreate()` never fires, and — since on-device testing found the stale-window-label failure mode can otherwise become permanent (every later attempt fails identically, recoverable only by force-stopping the app) — a last-resort self-triggered process restart (`Process.killProcess()` + an `AlarmManager`-scheduled relaunch) on the very first such failure. `AppHandle::restart()` (Tauri's own API) was evaluated and found unusable on Android: its desktop implementation re-execs the current binary via `Command::new(current_exe)`, which has no Android equivalent, so it would just `exit(0)` the process with nothing bringing it back.
- The Android source tree and generated Tauri Android tree must remain synchronized (`node scripts/copy-android-scaffolding.mjs`); a successful Gradle build, generated APK, or contract test alone is not sufficient evidence — every lifecycle change in this area needs a real-device test.

#### Suggested next steps

1. Continue real-device soak testing across the scenarios that have triggered the residual "webview creation silently never completes" issue (app-switch-triggered task reclamation, screen-off for an extended period) to build a larger sample of when the safety nets (watchdog / process restart) actually fire versus a clean rebuild.
2. If the residual issue needs to be eliminated rather than recovered from, the next investigation layer is `tauri-runtime-wry`'s own window-creation message dispatch (not `wry` itself, which is already vendor-patched) — confirming whether `WebviewWindowBuilder::build()`'s `Ok` return is ever a false positive there on other platforms too, or whether this is genuinely Android-specific.
3. Automate an ADB-driven regression loop for the scenarios above (repeated Logo taps across app-switch/screen-off cycles, verifying `onWebViewCreate()` fires or the watchdog/restart safety net recovers) so future changes in this area don't rely solely on manual real-device testing.
