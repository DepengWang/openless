...
The following five phases are the recommended handoff order. Do not start a later phase merely to mask a failure in an earlier one.

1. **Phase 1 — Reproduce and instrument the lifecycle failure (P0).** Repeatedly test logo/settings opens, background/foreground transitions, switching to Camera/Dialer/Alipay, and returning to OpenLess. Capture logcat and screenshots around Activity/WebView creation and destruction, Surface changes, process freezing/unfreezing, and warmup/background transitions. Keep one Tauri host; do not use a second Tauri runtime or a Rust-runtime restart as the first workaround.
2. **Phase 2 — Make black-screen recovery deterministic (P0).** Separate backend liveness from UI visibility and Surface/frame readiness. Correct stale Activity/WebView ownership and foreground recovery, then verify that a real non-black frame is visible rather than relying only on `postVisualStateCallback()`.
3. **Phase 3 — Restore recording and indicator state (P0).** Resynchronize recorder state, heartbeat, breathing indicator, and input-panel state after recovery. Correlate audio-recorder state with Activity/WebView/Surface and process events so a visually recovered app cannot retain a detached recording chain.
4. **Phase 4 — Automate regression and simplify lifecycle code (P1).** Add an ADB-driven smoke loop for logo/settings, app switching, foreground/background recovery, recording start/stop/cancel, IME enablement, and every insertion tier. Split runtime and input-panel code into smaller controllers/state machines without changing IME behavior. Keep the diagnostic panel feature-gated or clearly diagnostic-only for release builds.
5. **Phase 5 — Product and release hardening (P2).** Improve settings load performance, clipboard retention/privacy documentation, offline/provider fallback behavior, OEM battery-management guidance, accessibility, and cross-version WebView compatibility. Release only after repeated real-device tests record the device model, Android version, ABI, permissions, tested commit, and crash/exit evidence.
